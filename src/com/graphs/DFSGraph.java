package com.graphs;

import java.util.*;

public class DFSGraph {

    Map<Integer, List<List<Integer>>> adjacencyList;

    DFSGraph() {
        adjacencyList = new HashMap<>();
    }

    void insert(int start, int end, int distance) {
        List<List<Integer>> parentList;
        List<Integer> childList = new ArrayList<>();
        childList.add(end);
        childList.add(distance);
        if(adjacencyList.containsKey(start)) {
            parentList = adjacencyList.get(start);
        }
        else {
            parentList = new ArrayList<>();
        }
        parentList.add(childList);
        adjacencyList.put(start, parentList);
    }

    void printGraph() {
        for(int key: adjacencyList.keySet()) {
            System.out.println(key + " --> " + adjacencyList.get(key));
        }
    }

    void printDFS() {
        int size = adjacencyList.size();
        int[] visited = new int[size];
        List<Integer> sequence = new ArrayList<>();
        for(int eachNode: adjacencyList.keySet()) {
            for(int i=0;i<visited.length;i++) {
                visited[i] = 0;
            }
            visited[eachNode] = 1;
            sequence.add(eachNode);
            System.out.println("DFS for key: "+eachNode);
            System.out.print(eachNode + " -> ");
            DFSAllNodes(eachNode, visited, sequence);
            System.out.println();
        }

    }

    void DFSAllNodes(int current, int[] visited, List<Integer> sequence) {
        for(List<Integer> adjacents: adjacencyList.get(current)) {
            int nextNode = adjacents.get(0);
            if(visited[nextNode] == 1) {
                continue;
            }
            visited[nextNode] = 1;
            sequence.add(nextNode);
            System.out.print(nextNode + " -> ");
            DFSAllNodes(nextNode, visited, sequence);
        }
    }

    public static void main(String[] args) {
        DFSGraph graph = new DFSGraph();
        graph.insert(0, 2, 10);
        graph.insert(1, 3, 1);
        graph.insert(2, 0, 1);
        graph.insert(2, 4, 100);
        graph.insert(3, 2, 5);
        graph.insert(3, 4, 5);
        graph.insert(4, 1, 5);
        graph.insert(4, 0, 5);

        graph.printGraph();
        graph.printDFS();
    }

}
